...
char* nfs_get_fname(const char* path) {
    // strrchr函数返回一个指针，指向字符串中最后一个出现的字符'/'的位置(逆向搜索)
    char *q = strrchr(path, '/') + 1;
    return q;
}
 ...
 * 
 * @param offset 偏移量
 * @param out_content 输出缓冲区
 * @param size 读取大小
 * @return 成功返回NFS_ERROR_NONE，失败返回错误码
 */
...
struct nfs_inode* nfs_alloc_inode(struct nfs_dentry *dentry) {
    ...
    // 先按字节寻找空闲的inode位图
    for (; byte_cursor < NFS_BLKS_SZ(nfs_super.map_inode_blks); byte_cursor++) {
        // 再在该字节中遍历8个bit寻找空闲的inode位图
        for (; bit_cursor < UINT8_BITS; bit_cursor++) {
...
int nfs_alloc_data() {
    ...
    for (; byte_cursor < NFS_BLKS_SZ(nfs_super.map_data_blks); byte_cursor++) {
        // 再在该字节中遍历8个bit寻找空闲的数据位图
        for (; bit_cursor < UINT8_BITS; bit_cursor++) {
...
int nfs_sync_inode(struct nfs_inode *inode) {
    ...
    inode_d.block_allocated = inode->block_allocated;
    for (int i = 0; i < NFS_DATA_PER_FILE; i++) {
    ...
    if (NFS_IS_DIR(inode)) { // 如果当前inode是目录，那么数据是目录项，且目录项的inode也要写回                         
        ...
        int data_blks_num = 0;
        // 要将dentry的内容刷回磁盘，目录有block_allocated个数据块
        while ((dentry_cursor != NULL) && (data_blks_num < inode->block_allocated)) {
...
/**
 * @brief 读取指定inode的内容到内存
 * 
 * @param dentry dentry指向ino，读取该inode
 * @param ino inode唯一编号
 * @return 指向读取的inode结构体指针，失败返回NULL
 */
struct nfs_inode* nfs_read_inode(struct nfs_dentry *dentry, int ino) {
    ...
    inode->dentry = dentry;
    inode->dentrys = NULL;
    inode->block_allocated = inode_d.block_allocated;
    for (int i = 0; i < NFS_DATA_PER_FILE; i++) {
    ...
    // inode是普通文件则直接读取数据块
        ...
        for (int i = 0; i < NFS_DATA_PER_FILE; i++) {
            inode->data[i] = (uint8_t *)malloc(NFS_BLK_SZ());
            if (nfs_driver_read(NFS_DATA_OFS(inode->block_pointer[i]), (uint8_t *)inode->data[i], NFS_BLK_SZ()) != NFS_ERROR_NONE) {
                NFS_DBG("[%s] io error\n", __func__);
                free(inode);
                return NULL;
            }
 ...
 * 
 * @param path 路径字符串
 * @param is_find 是否找到完整路径标志
 * @param is_root 是否根目录标志
 * @return 指向找到的目录项指针
 */
...
int nfs_umount() {
    ...
    // 从根节点向下刷写节点 
    if (nfs_sync_inode(nfs_super.root_dentry->inode) != NFS_ERROR_NONE) {
        return -NFS_ERROR_IO;
    }
